...
output:
  html_document:
    ...
    toc_float: yes
params:
  w_dir: '/Volumes/spott/1_Shared_projects/LCL_Fiber_seq/QC/m84241_250117_150907_bc2107'
  sample: 'm84241_250117_150907_bc2107'
...
file_preffix = c(mosdepth = 'mosdepth.summary.txt',  
                 ...
                 byread = 'byRead.tsv', 
                 promoter = 'promoter_stats.tsv',
                 #nuc = 'nucleosome_stats.tsv',
                 fire = 'fire_stats.tsv'
                 )
...
is_nano = ifelse(tolower(params$platform) == 'asdfopore', TRUE, FALSE)
...
tab = cbind(tab, 
            data.frame(
              `Reads with no 6mA methylation` = sum(rd$FractionMethylatedA == 0) / tab$`Mapped, gDNA reads`,
              `Mean 6mA methylation fraction per read` = mean(rd$PercentageMethylatedA / 100), 
              `Median 6mA methylation fraction per read` = median(rd$PercentageMethylatedA / 100), 
              `Reads with no 5mC methylation` = sum(rd$FractionMethylatedC == 0) / tab$`Mapped, gDNA reads`,
              `Mean 5mC methylation fraction per read` = mean(rd$PercentageMethylatedC / 100), 
              `Median 5mC methylation fraction per read` = median(rd$PercentageMethylatedC / 100), 
              `Reads with no 5mC and 6mA methylation` = sum(rd$FractionMethylatedC == 0 & rd$FractionMethylatedA == 0) / tab$`Mapped, gDNA reads`,
            check.names = F)
            )
```
...
```{r, met_promoters_1, include = F}
## Load promoters stats. 
#x = read_tsv('../test_TSS.txt')[-8]
#names(x) = c('chrom', 'start', 'end', 'strand', 'TSS', 'Mean_methylation_prop','Coverage')
x = read_tsv(files$promoter)

x$Mean_methylation_prop[x$Mean_methylation_prop > 1] = 1
```


```{r, met_promoters_2, echo = F, warning = F}
tss_df = data.frame(
  `Number of promoters` = nrow(x), 
  `Promoters with no coverage` = sum(x$Coverage > 0),
  `Mean methylation proportion` = mean(x$Mean_methylation_prop[x$Coverage > 0]), 
  `Median methylation proportion` = median(x$Mean_methylation_prop[x$Coverage > 0]),
  `Fraction of promoters with no methylation` = sum(x$Mean_methylation_prop == 0 & x$Coverage > 0) / sum(x$Coverage > 0) , 
  check.names = F
) 
knitr::kable(tss_df, caption = 'Promoter methylation summary')

filter(x, Coverage > 0) %>%
ggplot(aes(x = Mean_methylation_prop)) + geom_histogram(binwidth = 0.01, fill = 'purple', color = 'grey80') + labs(x = 'Mean methylation proportion\n(6mA sites above threshold / region length)', title = '6mA Methylation in promoter regions\n(Based in TSS, -200, +50, strand aware)')

```


## FIRES: Lengths and methylation 

```{r nuclesome_oldfire, echo = F, eval = F}
## Read nucleosome counts and produce histogram. 

#x = read.table('../nuc_count.tsv', header = F)
x = read.table(files$nuc, header = F)
names(x) = c('n', 'length')

## Format data: Add bins. 
bins = c(seq(0, 1000, 25), seq(0, 100000, 5000)[-1])

x = arrange(x, length) %>% mutate(prop = n / sum(n)) %>% 
  mutate(bin = cut(length, breaks = bins, include.lowest = TRUE, right = FALSE))

x2= group_by(x, bin) %>% summarize(n = sum(n), prop = sum(prop))
x2$bin2 = bins[-1][ as.numeric(x2$bin) ]

## Summary statistics: Total nucleosomes and methylation. 
data.frame(
  `Total nucleosomes` = sum(x2$n), 
  `Mean nucleosome length` = mean(x2$bin2), 
  `Median nucleosome length` = median(x2$bin2), 
  `Q1` = quantile(x2$bin2, 0.25),
  `Q3` = quantile(x2$bin2, 0.75),
  check.names = F
) %>% knitr::kable(caption = 'Nucleosome summary')



A = ggplot(x2, aes(x = bin, y = prop)) + geom_col() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + labs(x = 'Nucleosome length (bp)', y = 'Proportion of nucleosomes', title = 'Nucleosome length distribution')

A %+% filter(x2, bin2 < 1000)

A %+% filter(x2, bin2 >= 1000)

```

Colorcode: FIREs with highest confidence are colored in darker red, with those with the lowest confidence in orange. 

```{r fire_oldversion, echo = F, eval = F}
## colors. 
cols = c('139,0,0' = '#8b0000', 
         '175,0,0' = '#af0000',
         '200,0,0' = '#c80000',
         '225,0,0' = '#e10000',
         '255,0,0' = '#ff0000',
...
#x = read.table('../fire.tsv', header = F)
x = read.table(files$fire, header = F)
names(x) = c('class', 'length', 'methylation')

x$class = factor(x$class, levels = c(names(cols)))

## Quick summary of total FIRES, and Fires per class. 
fire_df = data.frame(
  `Total FIRES` = nrow(x), 
  `Mean FIRES length` = mean(x$length), 
  `Median FIRES length` = median(x$length), 
  `Mean FIRES methylation` = mean(x$methylation), 
  `Median FIRES methylation` = median(x$methylation), 
  check.names = F
) 

knitr::kable(fire_df, caption = 'FIRES summary')

## Get the 80% of the lengths and use for x limit in the plot

ggplot(x, aes(x = length, color = class)) + geom_density() + 
  coord_cartesian(xlim = c(0, 500)) + scale_color_manual(values = cols) +
  labs(x = 'FIRE length', title = 'FIRE lengths per class')


ggplot(x, aes(x = methylation, fill = class)) + geom_histogram()  + 
  scale_fill_manual(values = cols) + labs(x = '6mA Methylation in FIRE', y = 'Number of FIRES', title = 'Methylation in FIREs')
...
#x = read.table('../fire.tsv', header = F)
x = read.table(files$fire, header = F)
names(x) = c('class', 'length')
x$class = gsub('^(.+),0$', '\\1', x$class)
x$class = factor(x$class, levels = c(names(cols)))
...
```{r final_summary_table, echo = F}
tab = as.matrix(tab) %>% t() %>% as.data.frame() 
names(tab) = sam_name

write.table(tab, file.path(params$w_dir, 'read_summary.tsv'), row.names = T)
knitr::kable(tab, caption = "Reads summary", digits = 3)
